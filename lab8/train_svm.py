...
def train_svm(training_points, kernel_fn=dot_product, max_iter=500,
    ...
    while iteration < max_iter:
        ...
        old_alphas = [pt.alpha for pt in training_points]
        ...
        for i in training_points:
            ...
            if show_graph and (animate or manual_animation):
                ...
                if animation_delay > 0 or manual_animation:
                    ...
                    if ([sv.name for sv in svm.support_vectors] == [sv.name for sv in old_support_vectors]
                ...
                if show_update:
                    if manual_animation:
                        ...
                        if animation_delay > 0:
                            ...
                            last_update_time = time()
                        update_svm_plot(svm)
            #print("Boundary drawn/updated. Number of support vectors: {}".format(len(svm.support_vectors)))

        iteration += 1

        # If alpha values have not changed *at all*, cease training
        if old_alphas == [pt.alpha for pt in training_points]:
            break
